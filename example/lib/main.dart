...
final endpoint = "https://zkihrm.cognitiveservices.azure.com/face/v1.0";
// Replace with your key
final key = "d6ecabfceda3480ea051b20ca3df4f49";
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _incrementCounter() async {
    ...
    if (_image != null) {
      _getImageSize(Image.file(File(_image.path), fit: BoxFit.fitWidth)).then((Size size) {
...
class _FaceDetectPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    for (var face in faces) {
      var width = face.faceRectangle?.width ?? 0 / _widthRatio;
      var height = face.faceRectangle?.height ?? 0 / _heightRatio;
      var left = offset.dx + (face.faceRectangle?.left ?? 0 / _widthRatio);
      var top = offset.dy + (face.faceRectangle?.top ?? 0 / _heightRatio);

      final _rect = Rect.fromLTWH(left, top, width.toDouble(), height.toDouble());
